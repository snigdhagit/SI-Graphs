...
class discrete_family(object):
    ...
    def _inter2Upper(self, observed, auxVar, alpha=0.05, tol=1e-6):
        ...
        if observed > self.sufficient_stat[self.n - 2] or (
                observed == self.sufficient_stat[self.n - 2] and auxVar == 1.):
    ...
    def _inter2Lower(self, observed, auxVar, alpha=0.05, tol=1e-6):
        ...
        """
        if observed > self.sufficient_stat[self.n - 1] or (
                observed == self.sufficient_stat[self.n - 1] and auxVar >= 1. - alpha):
